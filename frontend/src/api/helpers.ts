...
interface EnhancedMutationParamsWithoutBody<
  TData = unknown,
  TError = Error,
  TContext = unknown,
> extends Omit<
...
>({
  ...
  rMutationParams,
  options,
}: createPatchMutationHookArgs<ResponseSchema>) {
  return (params?: { query?: QueryParams; route?: RouteParams }) => {
    const queryClient = useQueryClient();
    const baseUrl = createUrl(endpoint, params?.query, params?.route);
    const mutationFn = async ({
    ...
    return useMutation({
      ...
      onSuccess: (data, context) =>
        rMutationParams?.onSuccess?.(data, context, queryClient),
      onError: (error, context) =>
        rMutationParams?.onError?.(error, context, queryClient),
